...
const apiFetch = async (path, options = {}) => {
  ...
  try {
    ...
    throw new Error("Backend server асаагүй байна. Spring Boot-ээ localhost:8080 дээр ажиллуулна уу.");
  }
...
const renderNotifications = async () => {
  ...
  try {
    ...
    container.innerHTML = notifications
      .map((notification) => `
        <article class="notification-card">
          ...
          <small>${notification.dateText}</small>
        </article>
      `)
      .join("");
  } catch (error) {
    container.innerHTML = `
      <article class="empty-state">
        <h3>Мэдэгдэл авах боломжгүй байна</h3>
        <p>${error.message}</p>
      </article>
    `;
  }
...
const handleProfileSave = async () => {
  ...
  try {
    currentStudent = await apiFetch(`/students/${currentStudent.code}/profile`, {
      method: "PUT",
      body: JSON.stringify({ name, phone })
    });
    setMessage(message, "Мэдээлэл хадгалагдлаа", "success");
  } catch (error) {
    setMessage(message, error.message, "error");
  }
};
...
const handleSurveySubmit = async () => {
  const course = document.getElementById("surveyCourse");
  const rating = document.getElementById("surveyRating");
  const comment = document.getElementById("surveyComment");
  const message = document.getElementById("surveyMsg");

  if (!course.value) {
    setMessage(message, "Хичээлээ сонгоно уу", "error");
    course.focus();
    return;
  }

  if (!rating.value) {
    setMessage(message, "Үнэлгээгээ сонгоно уу", "error");
    rating.focus();
    return;
  }

  if (!comment.value.trim()) {
    setMessage(message, "Саналаа бичнэ үү", "error");
    comment.focus();
    return;
  }
  ...
  try {
    ...
    rating.value = "";
    comment.value = "";
    setMessage(message, "Санал асуулга амжилттай илгээгдлээ", "success");
  } catch (error) {
    setMessage(message, error.message, "error");
  }
};
...
const handleLeaveSubmit = async () => {
  const date = document.getElementById("leaveDate");
  const reason = document.getElementById("leaveReason");
  const message = document.getElementById("leaveMsg");

  if (!date.value) {
    setMessage(message, "Чөлөө авах огноогоо сонгоно уу", "error");
    date.focus();
    return;
  }

  if (!reason.value.trim()) {
    setMessage(message, "Шалтгаанаа бичнэ үү", "error");
    reason.focus();
    return;
  }
  ...
  try {
    ...
    date.value = "";
    reason.value = "";
    setMessage(message, "Чөлөөний хүсэлт амжилттай илгээгдлээ", "success");
  } catch (error) {
    setMessage(message, error.message, "error");
  }
};
...
const handlePasswordChange = async () => {
  ...
  try {
    await apiFetch(`/students/${currentStudent.code}/password`, {
      method: "PUT",
...
const handleGlobalClick = (event) => {
  ...
  if (action === "openStudentMenu") {
    openStudentMenu();
    return;
  }

  if (action === "closeStudentMenu") {
    closeStudentMenu();
    return;
  }
...
const applyTheme = (theme) => {
  ...
  const menuToggle = document.getElementById("themeToggle");
  const loginToggle = document.getElementById("loginThemeToggle");

  if (menuToggle) {
    menuToggle.innerHTML = `<span>◐</span>Template: ${nextLabel}`;
  }

  if (loginToggle) {
    loginToggle.textContent = `${nextLabel} mode`;
  }
...
const init = () => {
  ...
  document.getElementById("submitSurveyButton").addEventListener("click", handleSurveySubmit);
  document.getElementById("submitLeaveButton").addEventListener("click", handleLeaveSubmit);
